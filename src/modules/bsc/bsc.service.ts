...
export class BscService {
  ...
  async getIndicators(filters: {
    ...
    const where: Prisma.BscIndicatorWhereInput = {
      ...
      ...(filters.objective
        ...
        : {}),
      ...(filters.perspective
        ? { objective: { perspective: { name: filters.perspective as any } } }
        : {}),
    };
  ...
  async getIndicatorByCode(code: string) {
    ...
    const yearTargetsByYear = new Map<number, number | null>(
      indicator.yearTargets.map((item) => [item.year, item.targetValue == null ? null : Number(item.targetValue)]),
    );
    const monthTargetsByKey = new Map<string, number | null>(
      indicator.monthTargets.map((item) => [`${item.year}-${item.month}`, item.targetValue == null ? null : Number(item.targetValue)]),
    );
    const monthActualsByKey = new Map<string, number | null>(
      indicator.monthActuals.map((item) => [`${item.year}-${item.month}`, item.actualValue == null ? null : Number(item.actualValue)]),
    );
    ...
    const monthly = Array.from(years)
      ...
      .flatMap((year) =>
        Array.from({ length: 12 }, (_, idx) => {
          ...
          const calc = computeStatus(target, actual, indicator.direction as any);
          return {
            year,
            month,
            target,
  ...
  async getManagement(year: number) {
    ...
    const rows = indicators.map((indicator) => {
      const targetByMonth = new Map(
        indicator.monthTargets.map((item) => [
          item.month,
          item.targetValue == null ? null : Number(item.targetValue),
        ]),
      );
      const actualByMonth = new Map(
        indicator.monthActuals.map((item) => [
          item.month,
  ...
  async getManagementSummary(year: number) {
    ...
    const grouped = new Map<string, { verde: number; amarelo: number; vermelho: number; semDados: number; total: number }>();
    for (const row of management.rows) {
      ...
      if (status === 'GREEN') current.verde += 1;
      else if (status === 'YELLOW') current.amarelo += 1;
      else if (status === 'RED') current.vermelho += 1;
  ...
  async setIndicatorMonthActual(code: string, payload: UpdateIndicatorMonthActualDto) {
    const indicator = await this.prisma.bscIndicator.findUnique({
      where: { code: code.toUpperCase() },
      select: { id: true, code: true },
    });
    if (!indicator) {
      throw new NotFoundException({ code: 'BSC_INDICATOR_NOT_FOUND', message: 'Indicador nao encontrado' });
    }
  ...
  async setIndicatorMonthTarget(code: string, payload: UpdateIndicatorMonthTargetDto) {
    const indicator = await this.prisma.bscIndicator.findUnique({
      where: { code: code.toUpperCase() },
      select: { id: true, code: true },
    });
  ...
  async getProjectTasks(projectId: string) {
    ...
    if (!project) throw new NotFoundException({ code: 'BSC_PROJECT_NOT_FOUND', message: 'Projeto não encontrado' });
    ...
    const tree = (parentWbs: string | null): any[] => {
