...
import { parseBudget2026CoordinatorWorkbook } from '../imports/budget-2026-proacao.importer';
...
function parseAccountCodeAndName(label: string) {
...
async function upsertCoordinator(
  ...
  return existingUser
    ? prisma.user.update({
        ...
        data: { name: sponsor, role: Role.COORDINATOR, active: true },
      })
    : prisma.user.create({
        data: {
          name: sponsor,
...
async function importDetailedProAcao2026(
  prisma: PrismaClient,
  buffer: Buffer,
  options: { defaultPassword?: string; createdById: string },
) {
  const parsed = parseBudget2026CoordinatorWorkbook(buffer);
...
async function importLegacyPlanningFormat(
  prisma: PrismaClient,
  buffer: Buffer,
  options: { defaultPassword?: string; createdById: string },
) {
...
export async function importPlanningXlsx(
  prisma: PrismaClient,
  buffer: Buffer,
  options: { defaultPassword?: string; createdById: string } = { createdById: '' },
) {
