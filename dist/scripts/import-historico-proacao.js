...
function resolveHistoricalFiles(root) {
    ...
    for (const year of [2020, 2021, 2022, 2023]) {
        const found = files.find((name) => {
            ...
            return normalized.includes(`proacao${year}`) || normalized.includes(`proa${year}`) || normalized.includes(`proacao${year}`);
        });
        if (found) {
...
async function ensureCoordinator(name, passwordHash) {
...
async function main() {
    ...
    const catalogFile = path.join(root, 'conta de cada coordenador.xlsx');
    ...
    const importUser = await ensureCoordinator('Importado', passwordHash);
    const importProacao = await prisma.proacao.upsert({
        where: { name: 'Importado' },
        update: {},
        create: { name: 'Importado' },
    });
    for (const entry of catalog) {
        const coordinator = await ensureCoordinator(entry.padrinho, passwordHash);
    ...
    const files = resolveHistoricalFiles(root);
    for (const item of files) {
        ...
        for (const sheetName of workbook.SheetNames) {
            ...
            for (let i = 1; i < rows.length; i += 1) {
                ...
                const coordinator = await ensureCoordinator(padrinhoName, passwordHash);
                const proacao = await prisma.proacao.upsert({
...
//# sourceMappingURL=import-historico-proacao.js.map