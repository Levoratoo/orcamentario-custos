...
(async () => {
    ...
    try {
        ...
        for (const r of rows) {
            ...
            if (t.includes('PIS S/DEPRECI') || t.includes('COFINS S/DEPRECI') || t === '(-)PIS' || t === '(-) PIS' || t === '(-)COFINS' || t === '(-) COFINS') {
                console.log(`${r.descricao} | code=${r.codigo ?? ''} | sort=${r.sortOrder ?? ''} | parent=${r.parentId ?? 'null'} | val=${Number(r.valoresPorMes?.[month]?.previsto ?? 0).toFixed(2)}`);
            }
        }
    }
    finally {
...
//# sourceMappingURL=tmp-tax-depr-rows.js.map