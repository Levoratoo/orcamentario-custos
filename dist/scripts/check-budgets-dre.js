...
(async () => {
    ...
    for (const year of [2024, 2025, 2026]) {
        ...
        for (const budget of budgets) {
            ...
            console.log(`${budget.id.slice(0, 8)} | ${budget.kind} | ${budget.status} | active=${budget.isActive} | ${budget.name} | PREV rows=${previstoCount} sum=${Number(previstoSum._sum.value ?? 0)} | REAL rows=${realizedCount} sum=${Number(realizedSum._sum.value ?? 0)}`);
        }
    }
    await prisma.$disconnect();
})();
//# sourceMappingURL=check-budgets-dre.js.map