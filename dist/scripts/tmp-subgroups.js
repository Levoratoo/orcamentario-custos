...
(async () => {
    ...
    try {
        ...
        for (const r of rows) {
            ...
            if (t.includes('DESPESAS COMERCIAIS -') || t.includes('DESPESAS COM VIAGENS -') || t.includes('SOFTWARES -') || t.includes('LOGISTICA -') || t.includes('OUTRAS DESPESAS -')) {
                console.log(r.descricao, '| parent', r.parentId ?? 'null', '| val', Number(r.valoresPorMes?.[month]?.previsto ?? 0));
            }
        }
    }
    finally {
...
//# sourceMappingURL=tmp-subgroups.js.map