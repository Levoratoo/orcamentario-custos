...
const planning_importer_1 = require("./planning.importer");
...
let PlanningService = class PlanningService {
    ...
    async listProacoes(user, userId) {
        const targetUserId = user.role === client_1.Role.ADMIN ? userId : user.sub;
        const where = {};
        if (targetUserId) {
        ...
        const withValueCount = await Promise.all(proacoes.map(async (proacao) => {
    ...
    async getGrid(user, proacaoId, year, userId) {
        ...
        if (!proacao) {
            throw new common_1.NotFoundException({ code: 'PROACAO_NOT_FOUND', message: 'Pro acao nao encontrada' });
        }
        const accountWhere = {};
        if (!isAll) {
    ...
    async updateValue(user, dto) {
        ...
        if (!account) {
            throw new common_1.NotFoundException({ code: 'ACCOUNT_NOT_FOUND', message: 'Conta nao encontrada' });
        }
        if (user.role !== client_1.Role.ADMIN) {
            ...
            if (!assignment) {
                throw new common_1.ForbiddenException({ code: 'FORBIDDEN', message: 'Sem permissao para editar esta conta' });
            }
        }
        ...
        if (existingValue?.locked && user.role !== client_1.Role.ADMIN) {
            throw new common_1.ForbiddenException({ code: 'LOCKED', message: 'Valor bloqueado para edicao' });
        }
    ...
    async finalize(user, dto) {
        ...
        if (!isAdmin && dto.userId && dto.userId !== user.sub) {
            throw new common_1.ForbiddenException({ code: 'FORBIDDEN', message: 'Sem permissao para este usuario' });
        }
        ...
        for (const accountId of accountIds) {
            for (let month = 1; month <= 12; month += 1) {
                ...
                ops.push(this.prisma.planningValue.upsert({
                    where: {
                        accountId_year_month: {
                            accountId,
                            year: dto.year,
    ...
    async importFromXlsx(file, createdById) {
        return (0, planning_importer_1.importPlanningXlsx)(this.prisma, file.buffer, { defaultPassword: '123456', createdById });
    }
    toNumber(value) {
    ...
    async getSummary(user, proacaoId, year, userId) {
        if (!proacaoId) {
            throw new common_1.NotFoundException({ code: 'PROACAO_REQUIRED', message: 'Pro acao nao informada' });
        }
        const isAll = proacaoId === 'all';
        const isAdmin = user.role === client_1.Role.ADMIN;
        const targetUserId = isAdmin ? userId : user.sub;
        ...
        const accountCodes = Array.from(new Set(accounts
        ...
        const sumSeries = (values) => values.reduce((acc, value) => acc + this.toNumber(value), 0);
        const fallbackPlannedCurrent = sumSeries(plannedCurrentSeries) === 0
            ? await this.getPlannedFromBudgetDreLinesByMonth(year, accountCodes)
            : null;
        const fallbackPlannedPrev = sumSeries(plannedPrevSeries) === 0
    ...
    async auditConsistency(user, proacaoId, year, userId) {
        if (!proacaoId) {
            throw new common_1.NotFoundException({ code: 'PROACAO_REQUIRED', message: 'Pro acao nao informada' });
        }
        const isAll = proacaoId === 'all';
        ...
        if (!budget) {
            throw new common_1.NotFoundException({ code: 'BUDGET_NOT_FOUND', message: `Budget ${year} nao encontrado` });
        }
        const dreTree = await this.dreService.getTree(budget.id, 'DRE');
        const dreRows = dreTree.rows;
        ...
        planningAccounts.forEach((account) => {
            ...
            this.months.forEach((month) => {
                ...
                const dreValue = matchingDreRows.reduce((sum, row) => sum + toNumber(row.valoresPorMes?.[monthKey]?.previsto), 0);
        ...
        dreRows.forEach((row) => {
            if (!row.parentId || !dreById.has(row.parentId))
                return;
    ...
    async getPlannedFromBudgetDreLinesByMonth(year, accountCodes) {
    ...
    async getRealizedFromDreByMonth(year, accountCodes, options) {
        if (accountCodes.length === 0)
            return new Array(12).fill(0);
        if (year === 2026) {
            const budget2026 = await this.prisma.budget.findFirst({
