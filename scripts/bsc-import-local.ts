...
import { BscExcelImportService } from '../src/modules/bsc/import/BscExcelImportService';
...
function collectFiles(): string[] {
  ...
  if (fs.existsSync(absoluteDir)) {
    for (const fileName of fs.readdirSync(absoluteDir)) {
      ...
      if (!/mapa\s*estrategico/i.test(fileName)) continue;
...
async function main() {
  ...
  const service = app.get(BscExcelImportService);
  ...
  try {
    ...
    for (const filePath of files) {
      ...
      const warnings = Array.isArray(importJob?.warnings) ? (importJob?.warnings as any[]) : [];
      ...
      warnings.slice(0, 10).forEach((warning, index) => {
        console.info(
          `  ${index + 1}. [${warning.sheetName ?? '?'}:${warning.rowIndex ?? '?'}] ${warning.message ?? ''}`,
        );
      });
    }
  } catch (error: any) {
