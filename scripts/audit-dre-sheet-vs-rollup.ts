...
import { invalidateDre2026SheetCache, getDre2026SheetData } from '../src/dre/dre-2026-sheet';
import { normalizeDreKey, sanitizeLabel } from '../src/dre/dre-utils';
...
function applySheetHierarchy(rows: Row[]): Row[] {
...
function rollupHierarchyMonthValues(rows: Row[], months: string[]): Row[] {
  if (rows.length === 0) return rows;
  const byId = new Map(rows.map((r) => [r.id, r]));
  const childrenByParent = new Map<string, Row[]>();
  rows.forEach((r) => {
    if (!r.parentId || !byId.has(r.parentId)) return;
  ...
  rows.forEach((r) => {
    if (!r.parentId || !byId.has(r.parentId)) walk(r);
  });
  return rows;
}
...
function run() {
  ...
  const sheet = getDre2026SheetData();
  if (!sheet) {
  ...
  const sheetRows: Row[] = sheet.entries
    .sort((a, b) => a.rank - b.rank)
    .map((entry) => {
      const valoresPorMes: Row['valoresPorMes'] = {};
      for (let m = 1; m <= 12; m++) {
  ...
  const originalValues = new Map<string, Record<string, { previsto: number; realizado: number }>>();
  sheetRows.forEach((r) => {
    const snapshot: Record<string, { previsto: number; realizado: number }> = {};
    MONTHS.forEach((mk) => {
  ...
  const preRollupValues = new Map<string, Record<string, { previsto: number; realizado: number }>>();
  ...
  const deepCopy = hierarchyRows.map((r) => ({
    ...
    valoresPorMes: Object.fromEntries(
      Object.entries(r.valoresPorMes).map(([k, v]) => [k, { ...v }]),
    ),
  }));
  ...
  const issues: Array<{
    ...
    childCount: number;
    month: string;
    sheetPrevisto: number;
    rolledPrevisto: number;
    deltaPrevisto: number;
    sheetRealizado: number;
    rolledRealizado: number;
    deltaRealizado: number;
  }> = [];
