...
import { mapDashboardData, DashboardModel } from '@/lib/dashboardMapper';
...
export function useDashboardData(
  ...
  }, [mainPrevisto.rows, mainPrevisto.summary, mainPrevisto.bySector, mainPrevisto.months, mainRealizado.rows, mainRealizado.summary, mainRealizado.bySector, mainRealizado.months, mainClosingMonth]);
  const main = mode === 'projetado' && mainProjected ? { ...mainPrevisto, ...mainProjected } : mainMode;

  const comparePrevisto = useDre({ budgetId: isProjetado ? compareBudgetId : null, mode: 'previsto', actualBudgetId: compareActualBudgetId });
  const compareRealizado = useDre({ budgetId: isProjetado ? compareBudgetId : null, mode: 'realizado', actualBudgetId: compareActualBudgetId });
  const compareMode = useDre({ budgetId: compareBudgetId, mode: isProjetado ? 'previsto' : mode, actualBudgetId: compareActualBudgetId });
  ...
  const compareProjected = useMemo(() => {
    ...
    return mergeProjected(
      { rows: comparePrevisto.rows, summary: comparePrevisto.summary, bySector: comparePrevisto.bySector, months: comparePrevisto.months },
      { rows: compareRealizado.rows, summary: compareRealizado.summary, bySector: compareRealizado.bySector, months: compareRealizado.months },
      compareClosingMonth,
    );
  }, [comparePrevisto.rows, comparePrevisto.summary, comparePrevisto.bySector, comparePrevisto.months, compareRealizado.rows, compareRealizado.summary, compareRealizado.bySector, compareRealizado.months, compareClosingMonth]);
  ...
  }, [main.rows, main.summary, main.months, main.bySector, compare.rows, compare.summary, compare.months, compare.bySector, year, mode]);

  return {
    model,
    loading: mainPrevisto.loading || mainRealizado.loading || comparePrevisto.loading || compareRealizado.loading || mainMode.loading || compareMode.loading,
    error: mainPrevisto.error || mainRealizado.error || comparePrevisto.error || compareRealizado.error || mainMode.error || compareMode.error,
    main,
    compare,
  };
}
