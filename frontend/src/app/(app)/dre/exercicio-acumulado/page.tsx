...
export default function DreExercicioAcumuladoPage() {
  const { apiFetch } = useApiClient();
  const router = useRouter();
  const params = useSearchParams();
  const queryYear = Number(params.get('year'));
  const queryCutoff = Number(params.get('cutoffMonth'));
  ...
  const exerciseQuery = useQuery({
    queryKey: ['dre-exercicio-acumulado', year, queryCutoff || null],
    queryFn: () => backend.getDreExerciseAccumulated(apiFetch, year, Number.isFinite(queryCutoff) ? queryCutoff : undefined),
    enabled: Number.isFinite(year),
  });

  const lastClosedMonth = exerciseQuery.data?.lastClosedMonth ?? 0;
  const cutoffMonth = exerciseQuery.data?.cutoffMonth ?? (lastClosedMonth || 1);
  ...
  const setQuery = (nextYear: number, nextCutoff: number) => {
    router.replace(`/dre/exercicio-acumulado?year=${nextYear}&cutoffMonth=${nextCutoff}`);
  };
        ...
        <CardContent className="space-y-5 p-6">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div className="space-y-1">
              ...
              <div className="text-xs uppercase tracking-[0.14em] text-[color:var(--accent)]/80">Analise Financeira</div>
              <h1 className="text-4xl font-semibold leading-tight text-foreground">Demonstracao do Resultado do Exercicio Acumulado</h1>
              ...
              <div className="flex flex-wrap items-center gap-2 pt-1">
                <Badge className="border border-border/70 bg-[color:var(--surface-2)] text-muted-foreground">
                  Corte ate {monthNames[Math.max(1, cutoffMonth) - 1]}
                </Badge>
            ...
            <div className="flex flex-wrap items-center gap-2">
              <Select value={String(year)} onValueChange={(value) => setQuery(Number(value), cutoffMonth)}>
              ...
              <Select value={String(cutoffMonth)} onValueChange={(value) => setQuery(year, Number(value))}>
                <SelectTrigger className="h-10 w-[150px] border-border/70 bg-[color:var(--surface-1)] shadow-sm">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
          ...
          <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
            <Card className="rounded-2xl border-border/70 bg-[color:var(--surface-1)]/95 shadow-sm">
              <CardHeader className="pb-2">
                <CardTitle className="text-xs uppercase tracking-wide text-muted-foreground">Acumulado Ano Atual</CardTitle>
              </CardHeader>
              <CardContent className="space-y-1 text-2xl font-semibold tabular-nums text-foreground">
                ...
                <div className="text-xs font-normal text-muted-foreground">Jan ate {monthNames[Math.max(1, cutoffMonth) - 1]}</div>
              </CardContent>
            </Card>

            <Card className="rounded-2xl border-border/70 bg-[color:var(--surface-1)]/95 shadow-sm">
              <CardHeader className="pb-2">
                <CardTitle className="text-xs uppercase tracking-wide text-muted-foreground">Acumulado Ano Anterior</CardTitle>
              </CardHeader>
              <CardContent className="space-y-1 text-2xl font-semibold tabular-nums text-foreground">
            ...
            <Card className="rounded-2xl border-border/70 bg-[color:var(--surface-1)]/95 shadow-sm">
              ...
              >
                ...
                <div className="text-xs font-normal text-muted-foreground">Diferenca acumulada</div>
              </CardContent>
            </Card>
          ...
          <Card className="rounded-2xl border-border/70 bg-[color:var(--surface-1)]/92 shadow-sm">
            <CardHeader className="pb-1">
              <div className="flex flex-wrap items-center justify-between gap-2">
                <div>
                  <CardTitle className="text-sm text-foreground">Top Variacoes (Delta R$)</CardTitle>
                  <p className="text-xs text-muted-foreground">Maiores impactos absolutos no acumulado do periodo.</p>
                </div>
                <div className="flex flex-wrap items-center gap-2">
                  <Badge className="border border-[color:var(--accent-border)] bg-[color:var(--accent-soft)] text-[color:var(--accent)]">
        ...
        <Card className="rounded-xl border-border/70 bg-[color:var(--surface-1)] shadow-sm">
          <CardContent className="p-4">
            <ExerciseComparisonTable
              ...
              previousLabel={`Acumulado ${exerciseQuery.data.compareYear} (jan-${monthNames[Math.max(1, cutoffMonth) - 1].toLowerCase()})`}
              currentLabel={`Acumulado ${exerciseQuery.data.year} (jan-${monthNames[Math.max(1, cutoffMonth) - 1].toLowerCase()})`}
            />
          </CardContent>
        </Card>
      ) : (
